...
class Game:
    ...
    def _init_sounds(self):
        ...
        self.sounds["coin"].set_volume(0.45)
        self.sounds["respawn"].set_volume(0.55)
        self.sounds["hit"].set_volume(0.35)
        self.sounds["level_clear"].set_volume(0.55)
        self.sounds["boost"].set_volume(0.45)
    ...
    def _handle_button_click(self, pos):
        ...
        # Check top-right buttons (like, esc)
        for button_name, button_rect in self.button_rects.items():
            if button_rect.collidepoint(pos):
                if button_name == "like":
                    # Like button action (can be extended)
                    self.sounds["coin"].play()
                elif button_name == "esc":
                    # ESC button returns to menu
                    self.return_to_menu = True
