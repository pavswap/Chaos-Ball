...
class Renderer:
    ...
    def _draw_hud(self, game):
        ...
        self._draw_top_right_buttons(game)
    ...
    def _draw_top_right_buttons(self, game):
        """Draw Like and ESC buttons on the top-right of the screen."""
        game.button_rects = {}  # Clear button rects for this frame
        
        button_width = 60
        button_height = 40
        button_y = 20
        gap = 10
        start_x = SW - (button_width * 2 + gap) - 20
        
        # ESC Button
        esc_rect = pygame.Rect(start_x, button_y, button_width, button_height)
        pygame.draw.rect(self.screen, (200, 60, 60), esc_rect)
        pygame.draw.rect(self.screen, (255, 100, 100), esc_rect, 2)
        esc_text = self.font_small.render("ESC", True, (255, 255, 255))
        self.screen.blit(esc_text, (esc_rect.centerx - esc_text.get_width()//2, 
                                     esc_rect.centery - esc_text.get_height()//2))
        game.button_rects["esc"] = esc_rect
        
        # Like Button
        like_rect = pygame.Rect(start_x + button_width + gap, button_y, button_width, button_height)
        pygame.draw.rect(self.screen, (255, 100, 150), like_rect)
        pygame.draw.rect(self.screen, (255, 150, 200), like_rect, 2)
        like_text = self.font_small.render("👍", True, (255, 255, 255))
        self.screen.blit(like_text, (like_rect.centerx - like_text.get_width()//2,
                                      like_rect.centery - like_text.get_height()//2))
        game.button_rects["like"] = like_rect
    ...
    def _draw_hints(self, game):
        ...
        if idx == 0 and game.total_deaths == 0:
            lines.append(("WASD / Arrows: Move  |  SPACE/W: Jump (×2)  |  LSHIFT / RSHIFT: Dash  |  ESC: Quit",
                          (140,140,160)))
    ...
    def _draw_overlay(self, game):
        if game.state == "level_clear":
            ...
            # Draw level clear buttons
            self._draw_level_clear_buttons(game)
    ...
    def _draw_level_clear_buttons(self, game):
        """Draw Go Back, Exit, and Next Level buttons on level clear screen."""
        button_width = 100
        button_height = 50
        button_gap = 20
        ...
        button_colors = {
            "go_back": ((100, 150, 255), (150, 180, 255)),
            "exit": ((255, 100, 100), (255, 150, 150)),
            "next_level": ((100, 255, 150), (150, 255, 180))
        }
        ...
        buttons = ["go_back", "exit", "next_level"]
        ...
        for i, button_name in enumerate(buttons):
            ...
            color, highlight_color = button_colors[button_name]
            pygame.draw.rect(self.screen, color, button_rect)
            pygame.draw.rect(self.screen, highlight_color, button_rect, 3)
            
            # Draw button text
            button_text = self.font_small.render(button_labels[button_name], True, (255, 255, 255))
            self.screen.blit(button_text, 
                           (button_rect.centerx - button_text.get_width()//2,
                            button_rect.centery - button_text.get_height()//2))
            
            # Store button rect for click detection
            game.button_rects[button_name] = button_rect